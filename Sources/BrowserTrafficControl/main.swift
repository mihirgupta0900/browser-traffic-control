...
final class AppModel: ObservableObject {
    ...
    func preview() { guard let url = URL(string: sampleURL), router.isValidWebURL(url) else { notice = "Enter a valid http:// or https:// URL"; return }; notice = "Matches: \(router.profile(for: url, settings: settings))" }
}
...
struct ContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            GroupBox { VStack(alignment: .leading, spacing: 12) { Label("Default destination", systemImage: "person.crop.circle").font(.headline); HStack { Text("Dia profile").foregroundStyle(.secondary); Picker("Dia profile", selection: $model.settings.defaultProfile) { ForEach(model.availableProfiles(for: model.settings.defaultProfile), id: \.self) { name in Text(name + (model.profiles.contains(name) ? "" : " (Unavailable)")).tag(name) } }.labelsHidden().frame(width: 220); Spacer(); Button { model.refreshProfiles() } label: { Image(systemName: model.profilesLoading ? "arrow.triangle.2.circlepath" : "arrow.clockwise") }.buttonStyle(.borderless).help("Refresh profiles from Dia"); Text(model.profileStatus).font(.caption).foregroundStyle(.tertiary) } } }.padding(.bottom, 18)
        ...
        }.padding(24).frame(minWidth: 650, minHeight: 560).sheet(isPresented: $showingAdd) { RuleEditor(rule: Rule(pattern: "https://github.com/company/**", profile: model.settings.defaultProfile), title: "Add Routing Rule", profiles: model.availableProfiles(for: model.settings.defaultProfile)) { rule in model.settings.rules.append(rule); model.save(); showingAdd = false } }
